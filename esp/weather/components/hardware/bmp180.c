...
static esp_err_t bmp180_master_write_slave(i2c_port_t i2c_num, uint8_t *data_wr,
                                           size_t size) {
  ...
  i2c_master_stop(cmd);
  esp_err_t ret = i2c_master_cmd_begin(i2c_num, cmd, 20);
  i2c_cmd_link_delete(cmd);
  return ret;
}

static esp_err_t bmp180_write_reg(i2c_port_t i2c_num, uint8_t reg,
                                  uint8_t cmd) {
...
static esp_err_t bmp180_master_read_slave(i2c_port_t i2c_num, uint8_t *data_rd,
                                          size_t size) {
  if (size == 0) {
    return ESP_OK;
  }
...
static esp_err_t bmp180_read_int16(i2c_port_t i2c_num, uint8_t reg,
                                   int16_t *value) {
...
static esp_err_t bmp180_read_uint16(i2c_port_t i2c_num, uint8_t reg,
                                    uint16_t *value) {
  esp_err_t err = bmp180_master_write_slave(i2c_num, &reg, 1);
  if (err == ESP_OK) {
    uint8_t data_rd[2] = {0};
...
static esp_err_t bmp180_read_uint32(i2c_port_t i2c_num, uint8_t reg,
                                    uint32_t *value) {
  esp_err_t err = bmp180_master_write_slave(i2c_num, &reg, 1);
  if (err == ESP_OK) {
    uint8_t data_rd[3] = {0};
...
static esp_err_t bmp180_read_uncompensated_temperature(int16_t *temp) {
  esp_err_t err =
      bmp180_write_reg(I2C_NUM_0, BMP180_CONTROL, BMP180_READ_TEMP_CMD);
  if (err == ESP_OK) {
...
static uint32_t bmp180_read_uncompensated_pressure(uint32_t *up) {
  esp_err_t err =
      bmp180_write_reg(I2C_NUM_0, BMP180_CONTROL,
                       BMP180_READ_PRESSURE_CMD + (oversampling << 6));
  if (err == ESP_OK) {
...
esp_err_t bmp180_read_altitude(uint32_t reference_pressure, float *altitude) {
  ...
  if (err == ESP_OK) {
    *altitude =
        44330 *
        (1.0 - powf(absolute_pressure / (float)reference_pressure, 0.190295));
  } else {
...
esp_err_t bmp180_init(int pin_sda, int pin_scl) {
  ...
  // err = i2c_set_timeout(I2C_NUM_0, 1000 / portTICK_PERIOD_MS);
  ...
  ESP_LOGI(TAG, "AC1: %d, AC2: %d, AC3: %d, AC4: %d, AC5: %d, AC6: %d", ac1,
           ac2, ac3, ac4, ac5, ac6);
