...
char *prepare_post_data() {
    ...
    char *post_data = "{\n\t\"pressure\": 21.0, \"temperature1\": 37.0, \"temperature2\": 1.0, \"rainDetected\": 2.0, \"humidity\": 3.0, \"lightIntensity\": 4.0, \"gasConcentration\": 5.0\n}";
...
void http_post_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (err == ESP_OK) {
            //ESP_LOGI(TAG, "HTTPS POST Status = %d, content_length = %d", esp_http_client_get_status_code(client), esp_http_client_get_content_length(client));
        } else {
